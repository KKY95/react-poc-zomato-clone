...
function ExploreCard(props) {
    const name = props.resturant?.info?.name ?? "";
    const coverImg = props.resturant?.info?.image?.url;
    const deliveryTime = props.resturant?.order?.deliveryTime;
    const rating = props.resturant?.info?.rating?.rating_text;
    const approxPrice = props.resturant?.info?.cfo?.text;
    const offers = props.resturant?.bulkOffers ?? [];
    const cuisines = props.resturant?.info?.cuisine?.map((item) => item.name).slice(0, 3);
    const bottomContainers = props.resturant?.bottomContainers;
    const goldOff = props.resturant?.gold?.text;
