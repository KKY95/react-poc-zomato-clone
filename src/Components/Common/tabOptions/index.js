    ...
    return (
        <div className="tab-options">
            <div className="max-width options-wrapper">
                {tabs.map((tab) => {
                    return (
                        <div onClick={() => props.updateActiveTab(tab.name)}
                            className={`tab-item absolute-center cur-po ${props.activeTab === tab.name && 'active-tab'}`}
                        >
                            <div className="tab-image-container absolute-center" style={{ backgroundColor: `${props.activeTab === tab.name ? tab.backdrop : ""}` }}>
                                <img className="tab-image" alt={tab.name} src={props.activeTab === tab.name ? tab.active_img : tab.inactive_img} />
                            </div>
